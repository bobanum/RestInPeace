...
class TableOrView {
    ...
    function get_foreign_key() {
        ...
        return preg_replace(array_map(fn($r) => '#' . $r . '$#', array_keys($rep)), $rep, $this->name) . '_id';
    }
    /**
    ...
    public function addRelation($relation) {
        if (isset($this->relations[$relation->name])) return;
    ...
    static function findForeignTable($tables, $fk) {
        foreach ($tables as $table) {
            if ($table->get_foreign_key() === $fk) return $table;
        }
        return null;
    }
}
